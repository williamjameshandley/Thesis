...
\begin{align}
  ds^2 &= \d{t}^2 - a{(t)}^2\left( \d{\chi}^2 + S_k^2{(\chi)} \d{\Omega} \right)
...
\end{equation}
where $d{X}^2 = \d{\chi}^2 + S_k^2(\chi) \d{\Omega}$.
...
\end{equation}
One can see from this that a flat universe $(\Omega=1)$ is an unstable point of equilibrium of these equations, providing that $1+3w>0$. For both matter (${w=0}$) and radiation (${w=\frac{1}{3}}$), the universe is rapidly driven away from flatness as the universe expands. The effect of traditional mass-energy is to increase the curvature of space.
...
However, if for some period $w<-1/3$ at some earlier time, then~\eqref{eqn:cos:instability} says that $\Omega=1$ is an attractor state. The Raychaudhuri equation shows that:
\begin{equation}
  \ddot{a} \propto -(1+3w)
...
The near-perfect isotropy of the CMB also presents a problem. 



\begin{equation}
  \eta = \int \frac{\d{t}}{a} = \int \frac{\d{z}}{H}
\end{equation}

Modelling as a single component universe with equation of state parameter $w$, equation~\eqref{eqn:cos:expansion_history} yield:
\begin{equation}
...
\end{equation}
represents the particle horizon, i.e.\ the furthest apart.
...
\begin{equation}
  H=\dot{\phi}=\text{const}\Rightarrow z=a,\quad a = \frac{1}{H(\eta_\mathrm{end}-\eta)} \propto e^{Ht}, \quad -\infty<\eta<\eta_\mathrm{end}.
  \label{eqn:cos:de-sitter}
\end{equation}
Thus, in a pure de-Sitter space, ${z^{\prime\prime}/z = 2{(\eta_\mathrm{end}-\eta)}^{-2}}$ and the differential equation~\eqref{eqn:cos:mode_func} has the general solution:
\begin{align}
...
\end{equation}
Re-writing $\varphi(\vx)$ as a Fourier synthesis yields:
\begin{align}
  \xi_\varphi(\vr) &= \mean{\int \frac{\d[3]{\vk^{\phantom\prime}}}{{(2\pi)}^3} \varphi(\vk)e^{i\vk\cdot\vx} \int \frac{\d[3]{\vk^{\prime}}}{{(2\pi)}^3} \varphi(\vk^{\prime})e^{i\vk^{\prime}\cdot(\vx+\vr)}}\\
  &= \int \frac{\d[3]{\vk^{\phantom\prime}}}{{(2\pi)}^3} \frac{\d[3]{\vk^{\prime}}}{{(2\pi)}^3} \mean{\varphi(\vk)\varphi(\vk^{\prime})}e^{i\vk\cdot\vx + i\vk^{\prime}\cdot(\vx+\vr)}
\end{align}
and taking the Fourier transform of both sides with respect to $\vx$ and $\vr$ yields:
\begin{equation}
...
To compute the power spectrum of comoving curvature perturbations $\mathcal{R}$, one therefore needs to compute the average on the left hand side of equation~\eqref{eqn:cos:power_spectrum_definition}. Quantum mechanically this is:
\begin{equation}
  \mean{\mathcal{R}_{\vk^{\phantom\prime}}\mathcal{R}_{\vk^{\prime}}} = 
  \frac{1}{z^2}\mean{v_{\vk^{\phantom\prime}}^{\phantom\dagger}v_{\vk^{\prime}}^{\dagger}} = 
  \frac{\chi_{k^{\phantom\prime}}^{\phantom\ast}\chi_{k^{\prime}}^\ast}{z^2}\mean{a_{\vk^{\phantom\prime}}^{\phantom\dagger}a_{\vk^{\prime}}^{\dagger}} = 
  {(2\pi)}^3\delta^{(3)}(\vk^{\phantom\prime}+\vk^{\prime}) {\left|\frac{\chi_k}{z}\right|}^2,
\end{equation}
giving:
\begin{equation}
...
\end{align}
Given that $H=\dot{\phi}=\mathrm{const}$, a de-Sitter inflationary phase therefore predicts a scale-invariant (Harrison-Zel'dovich) power spectrum. Now, true inflationary phases are not pure de-sitter, since $H$ is not quite constant. We may extend the de-Sitter result however by noting that at the time $t_*$ of horizon crossing  $k\approx a_*H_*$ the mode functions freeze out. For a given $k$ mode, the universe is still effectively de-Sitter at this point, with $z=a_*\dot{\phi}_*/H_*$. We may therefore evaluate equation~\eqref{eqn:cos:de-sitter_power} at horizon crossing to yield:
\begin{equation}
...
Different inflationary models predict different values of $A_s$ and $n_s$, with $n_s$ typically less than $1$, in contrast to the Harrison-Zel'dovich spectrum.
