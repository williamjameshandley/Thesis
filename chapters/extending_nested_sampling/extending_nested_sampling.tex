...
We must also keep track of the local and global evidence errors. Taking the square of equations~\eqref{eqn:ens:rr_ev} \&~\eqref{eqn:ens:rr_evi} yields:
%
\begin{align}
...
\end{align}
%
We can see that we're going to need to keep track of \(\{\overline{\ev X_p}, \overline{\ev_p X_p}, \overline{X_p^2}\}\) in addition to \(\{ \overline{\ev^2}, \overline{\ev_p^2} \}\). Taking various multiplications of equations~\eqref{eqn:ens:rr_ev},~\eqref{eqn:ens:rr_evi} \&~\eqref{eqn:ens:rr_Xi} finds:
%
\begin{align}
