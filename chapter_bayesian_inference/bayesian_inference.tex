...
To see this issue, consider the integration of a function along $[0,1]$
\begin{equation}
  \int_0^1 f(x) dx \approx \sum\limits_{i=0}^{n} f(x_i) w_i,
\end{equation}
where ${x_i\in[0,1]}$ are the quadrature points ${w_i\in\mathbb{R}}$ are the quadrature weights.\footnote{For example, if $x_i = \frac{i}{n}$ and $w_i=\frac{1}{n+1}$ then one obtains the left rectangle rule}
