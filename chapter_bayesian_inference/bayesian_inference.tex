...
\begin{equation}
  ...
  \label{eqn:bay:ehat}
\end{equation}
The central limit theorem states that this quantity is normally distributed with mean $\mean{\hat{E}_f} = \mean{f}$ and standard deviation $\sigma_f/\sqrt{n_s}$, where $\sigma_f$ is the true standard deviation $\sqrt{\mean{f^2}-\mean{f}^2}$. If $n_s\gg 1$ is large, then $\hat{E}_f$ will closely approximate the true mean. 

This approach has several advantages. If one has the set $S$ of samples~(\ref{eqn:bay:samples}), the computation~(\ref{eqn:bay:ehat}) is extremely cheap, in contrast to a full integral~(\ref{eqn:bay:quadrature_nd}). 
...
However, even if one can generate reliably and cheaply a full set $S$ of samples, this is not a full solution to the entire Bayesian inference problem problem. Computing means with it is only a reasonable methodology when $\sigma_f$ is small (or even defined at all). A very important example of a case where this is not true is the computation of the evidence integral~(\ref{eqn:bay:ev_short}).
