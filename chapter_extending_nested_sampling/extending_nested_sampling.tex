...
While it is possible to take equations~(\ref{eqn:ens:app_ev},\ref{eqn:ens:rr_x} \&~\ref{eqn:ens:Pt}) and compute the mean as a general formula~\citep{Keeton}, in the case of clustering this is uninformative. 
...
To estimate $\overline{\ev^2}$, we square~(\ref{eqn:ens:rr_Z}) and~(\ref{eqn:ens:rr_X}) and multiply both together to obtain:
%
\begin{align}
...
These should be initialised at $\{0,0,0,1,1\}$ respectively, and updated using equations~(\ref{eqn:ens:rr_Zb},\ref{eqn:ens:rr_Z2b},\ref{eqn:ens:rr_ZXb},\ref{eqn:ens:rr_Xb},\ref{eqn:ens:rr_X2b}) in that order. In fact, we keep track of the logarithm of these quantities, in order to avoid machine precision errors.
...
We must also keep track of the local and global evidence errors. Taking the square of equations~(\ref{eqn:ens:rr_ev} \&~\ref{eqn:ens:rr_evi}) yields:
%
\begin{align}
...
\end{align}
%
We can see that we're going to need to keep track of $\{\overline{\ev X_p}, \overline{\ev_p X_p}, \overline{X_p^2}\}$ in addition to $\{ \overline{\ev^2}, \overline{\ev_p^2} \}$. Taking various multiplications of equations~(\ref{eqn:ens:rr_ev},~\ref{eqn:ens:rr_evi} \&~\ref{eqn:ens:rr_Xi}) finds:
%
\begin{align}
