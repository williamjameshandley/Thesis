...
\newcommand{\eref}[1]{\nolinebreak(\ref{#1})}  % (X)
\newcommand{\Sref}[1]{Sec.\ \nolinebreak\ref{#1}}        % Section X
\newcommand{\Fref}[1]{Fig.\ \nolinebreak\ref{#1}}         % Figure X
...
In this paper, we focus primarily on the background dynamics of
...
\Sref{sec:When_is_kinetic_dominance_not_the_case?}.  We conclude in
\Sref{sec:Conclusions}. Section~\ref{sec:uniqueness_theorem} proves a
...
The simplest way to create a homogeneous and isotropic cosmological
...
Using the notation in \eref{eqn:equation_of_state} and defining the
present-day densities $\{\rho_{i,0}\}$, the evolution equations
\eref{eqn:Raychaudhuri_rho}--\eref{eqn:conservation} take the form 
%
\begin{align}
  \dot{H}+H^2 &= 
...
equations \eref{eqn:Raychaudhuri_mod} and
...
specified, the Friedmann equation \eref{eqn:Friedmann_mod} yields the
...
dynamical equations \eref{eqn:Raychaudhuri_mod} and
\eref{eqn:Klein_Gordon_mod} for $\phi(t)$ and $H(t)$.
...
slow-roll condition \eref{eqn:Slow-roll} were true,
%
\begin{equation}
  \dot\phi^2\gg V(\phi),
  \label{eqn:kddef}
...
\eref{eqn:kddef} is entirely generic as $a \to 0$, and holds
independently of the form of the potential $V(\phi)$.

From \eref{eqn:kddef} it is then possible to show that the Universe
...
until \Sref{sec:consequences_of_kinetic_dominance}. 

We shall now prove the generic nature of kinetic dominance, i.e.\ that
\eref{eqn:conditions} implies \eref{eqn:kddef}. The proof runs as
follows:
%
...
By differentiating the above definition, it is simple to show using
the Klein-Gordon equation \eref{eqn:Klein_Gordon_mod} that
%
\begin{align}
...
\end{align}
%
where $\dot{N}=H$ from Eq.\ \eref{eqn:e-folds}. The Hubble
parameter $\dot{N}$ can be related to the effective $e$-folds by
combining the Friedmann equation \eref{eqn:Friedmann_mod} with
Eq.\ \eref{eqn:Friedmann_c}:
%
\begin{equation}
...
Equations
\eref{eqn:rho_a},\eref{eqn:Friedmann_c}--\eref{eqn:N_H_relation} may
now be regarded as the evolution equations for the system in the
variables $\{N,\Nflat,\phi,\rho_i\}$.  The potential $V(\phi)$ now
only arises in \eref{eqn:Friedmann_c} in combination with $V_1$.
...
drop the $V_1$ part from \eref{eqn:Friedmann_c} and assume $V(\phi)$
is positive.
...
positive. Physically, \eref{eqn:tau_def} corresponds to choosing a
...
Under this rescaling of time, the modified evolution equations
\eref{eqn:Friedmann_c} and \eref{eqn:Raychaudhuri_c} take the form
%
\begin{align}
  \Nflatp^2 
  &= 
...
\begin{figure}
  ...
  \caption{The constraint provided by the definition of $\Nflat$ in
    ...
    lie on a hyperbola according to \protect\eref{eqn:Friedmann_tau}. A
    natural parametrization uses a hyperbolic angle $u$ detailed in
    \protect\eref{eqn:Ntrans} and \protect\eref{eqn:phitrans}. Note that
...
equations, we find that Eq.\ \eref{eqn:Friedmann_tau} is
trivially satisfied, and equation \eref{eqn:Raychaudhuri_tau} takes
the form
%
\begin{equation}
...
One can switch to the Hamilton-Jacobi representation by changing the
...
\end{equation}
%
which on applying to Eq.\ \eref{eqn:master_tau} yields
%
\begin{equation}
...
$\{\rho_i\}$ via \eref{eqn:N_H_relation} into the definition of
$\psi$. Under the transformation \eref{eqn:psi_by_phi}, the master
equation \eref{eqn:master_phi} takes the simple form
%
\begin{equation}
  \frac{du}{d\psi}  
  = 
...
\eref{eqn:Raychaudhuri_mod}--\eref{eqn:Klein_Gordon_mod} into a single
...
master equation \eref{eqn:master_eq} in the limit $a\rightarrow0$. We
...
\end{align}
%
From \Fref{fig:figure_hyperbola} it is easy to see that
%
\begin{equation}
  \left|\frac{\prm\phi}{\Nflatp}\right| > \sqrt{6}\m, 
\end{equation}
%
so combining this with Eq.\ \eref{eqn:div_psi} yields
%
\begin{equation}
...
decrease, one finds that if $u>0$, then $y>0$ by \eref{eqn:y_def} and
thus $\psi\to\infty$.\footnote{This explains the choice of sign in
  the parametrization \protect\eref{eqn:phitrans}}
...
\end{equation}
%
and by \eref{eqn:y_def}, we find that $y\ne0$ also.  The problem
...
equation \eref{eqn:master_eq} while remaining positive ($u>0$).

Let us assume that there exists a solution $f(\psi)$ of
\eref{eqn:master_eq} that is positive and finite ($0<f<\fm$ for some
finite $\fm$):
%
\begin{equation}
...
\eref{eqn:master_eq} it satisfies
%
\begin{equation}
...
\end{equation}
%
Taking the difference of \eref{eqn:master_eq_f} and
\eref{eqn:master_eq_h} gives :
%
\begin{equation}
...
from \eref{eqn:master_eq_diff} that the difference between $h$ and $f$
...
it is easy to see using \eref{eqn:master_eq_diff} that
%
\begin{equation}
  \frac{d}{d\psi}\left(h-f\right)
  > 
...
%
\begin{figure}
  \includegraphics[width=\textwidth]{uschem}
  \caption{In general there is at most a single solution $f(\psi)$ to
    the master equation \protect\eref{eqn:master_eq} that is positive
...
\Fref{fig:figure_uschem}. For any given potential $V(\phi)$, there is
...
diverges, then so does $u$ by Eq.\ \eref{eqn:y_def}. If $u$
...
\eref{eqn:Raychaudhuri_mod} and Friedmann \eref{eqn:Friedmann_mod}
...
$w$.  It is not difficult to show using Eq.\ \eref{eqn:rho_a},
...
Historically, inflationary potentials were considered in the context of
...
\eref{eqn:dotphidom}, such assumptions may now need revisiting.
...
\end{equation}
%
From Eq.\ \eref{eqn:H_of_a_1}, one can see that $(aH)^{-1}$ is
...
Moreover, from \eref{eqn:dotphidom}, the (dominant) energy density of
...
equations \eref{eqn:Raychaudhuri_mod} and \eref{eqn:Friedmann_mod} in
...
evolution equations take the form shown in \eref{eqn:Ray_KD_eq_rho} and
\eref{eqn:Friedmann_KD_eq_rho}. One can find solutions for
...
For specific values of $w$, the hypergeometric functions $_2F_1$ take
simple forms. If $w=-1$ or $0$, then Eq.\ \eref{eqn:t_of_a} is
expressible in closed form, in terms of trigonometric and algebraic
functions in $a$ respectively. If $w=-1/3$ or $1/3$, then
\eref{eqn:eta_of_a} may be expressed in closed form. In each of these
cases, these equations are invertible giving an expression for $a(t)$
or $a(\eta)$.  We note that, except for the case $w=-1/3$, the
solutions \eref{eqn:H_of_a}--\eref{eqn:eta_of_a} correspond to a
...
If $\rho_w=0$, then Eq.\ \eref{eqn:t_of_a} becomes
%
\begin{equation}
...
\end{equation}
%
and then substitute this into Eqs.\ \eref{eqn:H_of_a} and
\eref{eqn:dotphi_of_a} to find
%
\begin{align}
  H(t) 
  &= 
...
\end{equation}
%
since then, Eq.\ \eref{eqn:a_of_t_beta_flat} for $a(t)$ becomes
%
\begin{equation}
  a(t) 
  = 
...
\end{equation}
%
which is more in keeping with Eq.\ \eref{eqn:phi_of_t_flat}.  

For this case, one can also obtain analytical solutions in terms of
conformal time.  If $\rho_w=0$ and $\beta$ is defined by
\eref{eqn:beta_def}, Eq.\ \eref{eqn:eta_of_a} becomes
%
\begin{equation}
  \eta(t) 
  = 
...
\end{equation}
%
Using Eq.\ \eref{eqn:a_of_t_flat}, we can show
%
\begin{equation}
  \eta
  =
...
\end{equation}
%
Now we have $\eta(t)$ in Eq.\ \eref{eqn:eta_of_t}, we can change
Eqs.\ \eref{eqn:H_of_t_flat}, \eref{eqn:dotphi_of_t_flat},
\eref{eqn:phi_of_t_flat} and \eref{eqn:a_of_t_flat} to
%
\begin{align}
  a(\eta)
  &=
...
\end{equation}
%
For $w=-1$, Eq.\ \eref{eqn:t_of_a} is expressible in terms of
...
remaining equations \eref{eqn:H_of_a}, \eref{eqn:dotphi_of_a} and
\eref{eqn:phi_of_a} can be used to express the rest of the variables
...
\end{equation}
%
For $w=-1/3$, Eq.\ \eref{eqn:eta_of_a} is expressible in terms of
trigonometric functions, and may be rearranged to express the scale
...
of matter at the epoch $\Rp$.  For $w=0$, Eq.\ \eref{eqn:t_of_a}
...
is obtained, the remaining equations \eref{eqn:H_of_a},
\eref{eqn:dotphi_of_a} \& \eref{eqn:phi_of_a} can be used to express
...
\eref{eqn:eta_of_a} is expressible as an algebraic function, and may
be rearranged to express the scale factor $a$ in terms of coordinate
time. Once $a(\eta)$ is obtained, the remaining equations
\eref{eqn:H_of_a}, \eref{eqn:dotphi_of_a} and \eref{eqn:phi_of_a} can
...
--\eref{eqn:Friedmann_KD_eq_rho}, one would expect four constants of
...
\eref{eqn:tl}, \eref{eqn:etak}, \eref{eqn:tm} and \eref{eqn:etar}. If
...
We shall now demonstrate the utility of kinetic initial conditions in
...
\eref{eqn:H_of_t_flat} and \eref{eqn:phi_of_t_flat} at sufficiently
...
For $\rho_w=0$ the Universe is spatially flat and contains only the
...
form given in Eqs.\ \eref{eqn:H_of_t_flat} and
\eref{eqn:phi_of_t_flat}. As discussed in \Sref{sec:constants}, we may
...
We note that the evolution equations \eref{eqn:Friedmann_eq} \&
\eref{eqn:Motion_eq_Scalar} are invariant under  the simultaneous
...
\eref{eqn:Friedmann_eq} \& \eref{eqn:Motion_eq_Scalar} numerically.
Our kinetic initial conditions are chosen using Eq.\ 
\eref{eqn:phi_ic}--\eref{eqn:H_ic} with an initial time $\ti$ small
...
%
\begin{figure}
  \includegraphics[width=\textwidth]{Hpol}
...
%
\begin{figure}
  \includegraphics[width=\textwidth]{Kpol} 
  ...
  \protect\Fref{fig:figure_Hpol}.  }
    \label{fig:figure_Kpol}
\end{figure}
%

The evolution of the Hubble parameter is shown in
\Fref{fig:figure_Hpol} for polynomials with $n=2,4,6$. It is helpful
...
\end{equation}
%
This is used as a diagnostic tool in \Fref{fig:figure_Kpol} for the
quadratic potential $V(\phi) = \mu^2\phi^2$. Examining
\Fref{fig:figure_Kpol} one can see that our earlier expectations are
...
\end{equation}
%
Exponential potentials \eref{eqn:exp_pot} have been well studied
...
\eref{eqn:pow_law_a_sol}--\eref{eqn:pow_law_H_sol} appear to represent
...
\Sref{sec:The_generic_nature_of_kinetic_dominance}. 

In terms of the master equation \eref{eqn:master_eq}, in a flat
universe, one finds $\phi = \sqrt{\frac{2}{3}}\m \psi + \mathrm{const}
$ and thus for the exponential potential \eref{eqn:exp_pot} 
%
\begin{equation}
...
\Sref{sec:The_generic_nature_of_kinetic_dominance}, we know that this
...
\Sref{sec:The_generic_nature_of_kinetic_dominance} demonstrates that
Halliwell's methodology is applicable more generally.

%
\begin{figure}
  \includegraphics[width=\textwidth]{Hlam}
  \caption{As in \protect\Fref{fig:figure_Hpol}, but for the
  hyperbolic cosine potential \protect\eref{eqn:coshpot}, which tends
...
%
\begin{figure}
  \includegraphics[width=\textwidth]{Klam}
  \caption{As in \protect\Fref{fig:figure_Kpol}, but for the
...
We show the evolution of the Universe governed by the hyperbolic
cosine inflaton potential~\eref{eqn:coshpot} in
Figs.\ ~\ref{fig:figure_Hlam} \&~\ref{fig:figure_Klam}. The analysis
is the same as that presented in \Sref{sec:section_polynomial_potentials}.
...
The $f(\psi)$ described above in Eq.\ \eref{eqn:uf_power_law} is
...
By reverse-engineering the master equation \eref{eqn:master_eq_f}, one
...
\end{equation}
whose shape is detailed in \Fref{fig:figure_ospot}.
%
...
\eref{eqn:master_eq} one may show that the perturbed solution
satisfies:
%
\begin{align}
  \frac{d}{d\phi}\delta 
...
Working from Eq.\ \eref{eqn:perturb}, one also finds
%
\begin{align}
  \frac{d}{d\phi}\delta 
  <&
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{uf}
  \caption{An oscillating finite solution to the master equation
    \protect\eref{eqn:master_eq} with $V(\psi)$ defined as in Eq.\
    \protect\eref{eqn:Vphi_uf_example} and demonstrated in
    \protect\Fref{fig:figure_ospot}. If $a$ and $b$ are chosen such
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{ospot}
...
\Fref{fig:experimental_power_spectrum} has been taken from
...
The shape of the primordial power spectra obtained corresponds to that
...
\eref{eqn:curvature_power_spectrum} outside the slow-roll regime for
...
We demonstrate the suppression on large angular scales of the CMB and
late-time matter power spectra qualitatively in \Fref{fig:figure_Cl}.
...
from \Fref{fig:figure_CSpol} (for $n=2$), for which the axes were
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{powerspec}
...
%
\begin{figure}
  \includegraphics[width=\textwidth]{CSpol}
  \caption{The approximate power spectrum of the primordial curvature
    ...
    \protect\eref{eqn:curvature_power_spectrum}. The units of the $k$
    ...
    \protect\Fref{fig:experimental_power_spectrum}.  }
...
%
\begin{figure}
  \includegraphics[width=\textwidth]{CSlam}
  \caption{As in \protect\Fref{fig:figure_CSpol}, but for exponential
...
%
\begin{figure}
  \centerline{\includegraphics[width=\textwidth]{Cl}}
  \centerline{\includegraphics[width=\textwidth]{matter}}
...
To address these issues further, we consider in more detail the main
...
\Fref{fig:figure_BVS_initial_conditions} illustrates this
...
%
\begin{figure}
  \includegraphics[width=\textwidth]{newpot}
...
equipartition condition \eref{eqn:bvsbc} is satisfied. Thus, rather
than considering \eref{eqn:bvsbc} as a fundamental physical principle,
...
demanding that \eref{eqn:bvsbc} is satisfied at $\phi=0$ corresponds
...
\Fref{fig:figure_BVS_initial_conditions}).  If, however, $\phip$ is
...
which the Friedmann \eref{eqn:Friedmann_mod} and Klein-Gordon
\eref{eqn:Klein_Gordon_mod} equations take the form:
%
\begin{align}
  H^2 
  &=
...
By examining the Klein-Gordon equation
\eref{eqn:Klein_Gordon_dotphi_to_zero_no_fluid}, one can see that the
...
One can demonstrate the above statement formally by considering
Eqs.\ \eref{eqn:Klein_Gordon_dotphi_to_zero_no_fluid} and
\eref{eqn:Friedmann_dotphi_to_zero_no_fluid} in the Hamilton-Jacobi
representation:
%
\begin{align}
...
\end{equation}
%
Solutions to this equation are plotted in \Fref{fig:figure_edS}, which
demonstrates the following facts: 
%
\begin{itemize}
  \item There is a region in which solutions cannot exist, since by
    the Friedmann equation
    \eref{eqn:Friedmann_dotphi_to_zero_no_fluid} we find that
    $H>\sqrt{V(\phi)/3\m^2}$.
  \item  By Eq.\ \eref{eqn:Hamilton_Jacobi_H} that solutions meet
    this region with zero gradient.
  \item Outside of this region, the right-hand side of Eq.\ 
    \eref{eqn:Hamilton_Jacobi_H} is Lipschitz continuous (see
...
Consider the eternal de Sitter solution $\HedS(\phi)$, plotted
as the right-hand half of the dotted line in \Fref{fig:figure_edS}.
...
$\HedS$ and $\Hh$ satisfy Eq.\ \eref{eqn:Hamilton_Jacobi_H}.
...
\Fref{fig:figure_edS} reveals an interesting second eternal de Sitter
...
%
\begin{figure}
  \includegraphics[width=\textwidth]{edS}
  \caption{Schematic of the Hubble parameter against $\phi$ in the
    ...
    \protect\Fref{fig:figure_BVS_initial_conditions}. The solid curves
    represent the portions of the solutions to
    \protect\eref{eqn:Hamilton_Jacobi_H} with $\dot{\phi}\ge0$. The
...
the Friedmann \eref{eqn:Friedmann_mod} and Klein-Gordon
\eref{eqn:Klein_Gordon_mod} equations, with a single fluid $\rho_w$:
%
\begin{align}
  H^2
  &=
...
\eref{eqn:Friedmann_dotphi_to_zero} to show that
%
\begin{equation}
  H = \frac{2}{3(1+w) t}
  \quad
...
\end{equation}
%
We can now solve \eref{eqn:Klein_Gordon_dotphi_to_zero} for $\phi(t)$.
...
and $\phi(t)$ are given by Eqs.\ \eref{eqn:dotphi_of_t_flat} and
\eref{eqn:phi_of_t_flat}. We may solve these to find the relationship
...
For illustration, we applied this ``kinetic'' procedure for setting
...
the master equation \eref{eqn:master_eq}:
%
\begin{align}
  \frac{dy}{d\psi}
  &=
...
$[\psiz,\psi_1]$: From assumption \eref{eqn:conditions}, we know
$\dot{\phi}^2>\vellim^2>0$. If we unpack the definition of $y$ using
Eqs.\ \eref{eqn:y_def}, \eref{eqn:Ntrans}, \eref{eqn:tau_def} and
\eref{eqn:Friedmann_c}, one finds
%
\begin{equation}
  y 
  = 
...
If one assumes that $y(\psi)$ and $z(\psi)$ are two distinct
...
\end{equation}
%                                              
Applying Lipschitz continuity \eref{eqn:Lipschitz} to the inequality
in \eref{eqn:ineq_1} gives    
%
\begin{equation}
  \abs{y(\psi)-z(\psi)} 
  \le
...
\end{equation}
%
Applying this inequality back into \eref{eqn:ineq_2} shows
%
\begin{equation}
  \abs{y(\psi)-z(\psi)} 
  \le
...
\end{equation}
%
Applying this back into \eref{eqn:ineq_2} yields
%
\begin{equation}
  \abs{y(\psi)-z(\psi)} 
  \le
